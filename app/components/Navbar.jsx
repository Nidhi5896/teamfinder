  ...
  const handleAccept = async (notificationId) => {
    ...
    try {
      const response = await fetch("/api/acceptRequest", {
        ...
        body: JSON.stringify({ notificationId, action: "accept" }),
      });
      ...
      const statusResponse = await fetch("/api/notificationstatus", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setNotificationStatus({ [notificationId]: statusData.status });
    } catch (error) {
  ...
  const handleDeny = async (notificationId) => {
    try {
      const response = await fetch("/api/acceptRequest", {
        ...
        body: JSON.stringify({ notificationId, action: "deny" }),
      });
      ...
      console.error("Error denying notification:", error.message);
    }
  };
        ...
        {user ? (
          <>
            <button
              ...
              className="text-white hover:text-gray-400 focus:outline-none mr-4"
            >
            ...
            <Link href="/api/auth/logout">
              <p className="text-white hover:text-gray-400 focus:outline-none cursor-pointer">
                Sign out
              </p>
            </Link>
          </>
                ...
                <li key={index}>
                  {notification.projectowneremail === user?.email && (
                    <div>
                      ...
                      <div className="mt-2">
                        {notificationStatus[notification.id] === "accepted" && (
                          <p>You accepted this request.</p>
                        )}
                        {notificationStatus[notification.id] === "denied" && (
                          <p>You denied this request.</p>
                        )}
                        {!notificationStatus[notification.id] && (
                          <>
                            <button
